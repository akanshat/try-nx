...
const renderTime = (runTime, estimatedTime, width) => {
    ...
    const renderedTime = estimatedTime && runTime >= estimatedTime + 1
        ? chalk.bold.yellow(jest_util_1.formatTime(runTime, 0))
        : jest_util_1.formatTime(runTime, 0);
    let time = chalk.bold(`Time:`) + `        ${renderedTime}`;
    if (runTime < estimatedTime) {
        time += `, estimated ${jest_util_1.formatTime(estimatedTime, 0)}`;
    }
...
const getSummary = (aggregatedResults, options) => {
    ...
    const snapshots = chalk.bold('Snapshots:   ') +
        ...
        (snapshotsFilesRemoved && !snapshotsDidUpdate
            ? chalk.bold.yellow(jest_util_1.pluralize('file', snapshotsFilesRemoved) + ' obsolete') + ', '
            : '') +
        (snapshotsFilesRemoved && snapshotsDidUpdate
            ? chalk.bold.green(jest_util_1.pluralize('file', snapshotsFilesRemoved) + ' removed') + ', '
            : '') +
        (snapshotsUpdated
