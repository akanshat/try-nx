...
function determineUncoveredJestProjects(existingProjects) {
    const coveredJestProjects = get_jest_projects_1.getJestProjects().reduce((acc, key) => {
        acc[key] = true;
        return acc;
    }, {});
...
function updateBaseJestConfig(tree, baseJestConfigPath = 'jest.config.js') {
    ...
    const currentConfig = functions_1.jestConfigObject(tree, baseJestConfigPath);
    ...
    update_config_1.removePropertyFromJestConfig(tree, baseJestConfigPath, 'projects');
    update_config_1.addPropertyToJestConfig(tree, baseJestConfigPath, 'projects', determineProjectsValue(uncoveredJestProjects), { valueAsString: true });
    update_config_1.addImportStatementToJestConfig(tree, baseJestConfigPath, `const { getJestProjects } = require('@nrwl/jest');`);
    return;
}
