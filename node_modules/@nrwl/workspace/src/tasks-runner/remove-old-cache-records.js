...
function terminalOutputs() {
    try {
        return fs_1.readdirSync(path_1.join(folder, 'terminalOutputs')).map((f) => path_1.join(folder, 'terminalOutputs', f));
    }
    catch (e) {
        return [];
    }
...
function cachedFiles() {
    try {
        return fs_1.readdirSync(folder)
            .filter((f) => !f.endsWith('terminalOutputs'))
            .map((f) => path_1.join(folder, f));
    }
    catch (e) {
        return [];
    }
...
function removeOld(records) {
    try {
        ...
        records.forEach((r) => {
            try {
                const s = fs_1.statSync(r);
                if (time - s.mtimeMs > WEEK_IN_MS) {
                    if (s.isDirectory()) {
                        try {
                            fs_extra_1.removeSync(`${r}.commit`);
                        }
                        catch (e) { }
                    }
                    fs_extra_1.removeSync(r);
                }
            }
            catch (e) { }
        });
