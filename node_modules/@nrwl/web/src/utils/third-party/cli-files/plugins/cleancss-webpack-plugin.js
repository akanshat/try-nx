        ...
        hook(compiler, (compilation, chunks) => {
            ...
            chunks.forEach((chunk) => {
                if (isWebpack5) {
                    if (chunk.files && chunk.files.size > 0) {
                        const xs = Array.from(chunk.files.values());
                        files.push(...xs);
                    }
                }
                else {
            ...
            const actions = files
                ...
                if (output.errors && output.errors.length > 0) {
                    output.errors.forEach((error) => compilation.errors.push(new WebpackError(error)));
                    return;
                }
                ...
                let newSource;
                if (output.sourceMap) {
                    newSource = new webpackSources.SourceMapSource(output.styles, file, 
                    // tslint:disable-next-line: no-any
                    output.sourceMap.toString(), content, 
