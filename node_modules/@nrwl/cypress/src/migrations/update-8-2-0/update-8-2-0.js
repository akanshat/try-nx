            ...
            project.targets.forEach((target) => {
                ...
                rules.push(workspace_1.updateJsonInTree(target.options.cypressConfig, (json) => {
                    function getUpdatedPath(path) {
                        ...
                        return `./${core_1.normalize(path_1.relative(path_1.dirname(target.options.cypressConfig), path_1.relative(`./${tsConfig.options.outDir}`, path_1.join(path_1.dirname(target.options.cypressConfig), path))))}`;
                    }
                    ...
                    if (config.cypressConfig &&
                        ...
                        rules.push(workspace_1.updateJsonInTree(config.cypressConfig, (json) => {
                            function getUpdatedPath(path) {
                                ...
                                return `./${path_1.relative(path_1.dirname(config.cypressConfig), path_1.relative(`./${tsConfig.options.outDir}`, path_1.join(path_1.dirname(config.cypressConfig), path)))}`;
                            }
