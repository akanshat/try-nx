...
function removeTypescriptPlugin(tree) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        for (const [_, proj] of devkit_1.getProjects(tree)) {
            ...
            for (const target of cypressTargets) {
                ...
                for (const config of cypressConfigs) {
                    const cypressConfig = devkit_1.readJson(tree, config);
                    if (cypressConfig.pluginsFile) {
                        let pluginPath = path_1.join(path_1.dirname(config), cypressConfig.pluginsFile);
...
function removePreprocessor(tree, pluginPath) {
    ...
    const sourceFile = typescript_1.createSourceFile(pluginPath, pluginContents, typescript_1.ScriptTarget.ESNext, true);
    const deletions = [];
    const callback = (node) => {
        ...
        if (typescript_1.isCallExpression(node) &&
            ...
            let n = node.parent;
            while (!typescript_1.isExpressionStatement(n) && n === sourceFile) {
                n = n.parent;
            }
            deletions.push({
    ...
    tree.write(pluginPath, devkit_1.applyChangesToString(pluginContents, deletions));
}
