...
function getPackageVersion(json, packageName) {
    ...
    if (packageVersion) {
        return version_utils_1.checkAndCleanWithSemver(packageName, packageVersion);
    }
    else {
        return null;
    }
    ...
    devkit_1.updateJson(tree, 'package.json', (json) => {
        ...
        function updatePackageSection(storybookPackageName, packageSection) {
            if (json[packageSection][storybookPackageName]) {
                const version = version_utils_1.checkAndCleanWithSemver(storybookPackageName, json[packageSection][storybookPackageName]);
                if (semver_1.gte(version, '6.0.0') && semver_1.lt(version, targetStorybookVersion)) {
        ...
        if (storybookAngularVersion && semver_1.gte(storybookAngularVersion, '6.0.0')) {
            const angularVersion = getPackageVersion(json, '@angular/core');
            if (angularVersion && semver_1.gte(angularVersion, '12.0.0')) {
...
function migrateKnobsRegistration(tree) {
    ...
    const json = devkit_1.readJson(tree, 'package.json');
    const storybookKnobVersion = getPackageVersion(json, '@storybook/addon-knobs');
    if (storybookKnobVersion && semver_1.gte(storybookKnobVersion, '6.3.0')) {
        ...
        projects.forEach((projConfig, projName) => {
            ...
            const storybookMainJS = devkit_1.joinPathFragments(projConfig.root, '.storybook/main.js');
